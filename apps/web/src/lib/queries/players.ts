...
export async function fetchPlayerById(
  ...
  if (playerResult.error) {
    ...
    return {
      data: null,
      error: {
        message: isNotFound ? `Player not found: ${id}` : playerResult.error.message,
        code: playerResult.error.code,
      },
    }
  }

  if (statsResult.error) return { data: null, error: toQueryError(statsResult.error) }
...
export async function fetchPlayerByExternalId(
  ...
  if (playerError) {
    ...
    return {
      data: null,
      error: {
        message: isNotFound ? `Player not found: ${externalId}` : playerError.message,
