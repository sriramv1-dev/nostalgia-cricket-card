  ...
  try {
    ...
    try {
      ...
      if (message === 'TRUNCATED_RESPONSE') {
        return NextResponse.json(
          { error: ERROR_MESSAGES.TRUNCATED_RESPONSE, code: 'TRUNCATED_RESPONSE' },
          { status: 422 }
        )
      }
    ...
    if (parsed.ambiguous === true) {
      return NextResponse.json({ ambiguous: true, candidates: parsed.candidates })
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: ERROR_MESSAGES.DB_INSERT_FAILED, code: 'DB_INSERT_FAILED', debug: msg },
        { status: 500 }
      )
    }
